...
var js = function (tree) {
    switch (tree.type) {
        case "assign":
            return "let ".concat(js(tree.left), "=").concat(js(tree.right));
        case "call":
        ...
        case "number":
            return tree.value;
        case "program":
            return "{".concat(tree.prog.map(js).join(";"), "}");
        case "string":
            return "\"".concat(tree.value, "\"");
        case "if":
            return "((".concat(js(tree.condition), ") ? (").concat(js(tree.ifBranch), ") : (").concat(js(tree.elseBranch), "))");
    }
};
